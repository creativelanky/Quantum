...
export default function Navbar() {
    ...
    return (
        <header className={scrolled ? 'inset-x-0 fixed top-0 bg-[#4F0714] z-50 translate-y-0 transition-all ease-linear duration-500' : 'inset-x-0 absolute s top-10 z-50'}>
            ...
            <nav className="flex items-center justify-between lg:px24 md:px-16 px-8 bg-transparent">
                <div className={`relative z-50 flex items-center justify-between w-full md:w-auto ${scrolled && 'bg-[#4F0714]'}`}>
                    <div>
                ...
                <div className={`fixed md:relative md:bg-transparent md:h-fit h-60 w-screen inset-x-0 px-8 pb-10 md:pb-0 md:px-0 z-40 top-0 bg-[#4F0714] md:block flex items-end justify-center md:translate-y-0 md:w-auto ${open ? 'translate-y-0' : '-translate-y-[50vh]'} transition-transform ease-linear duration-500`}>
                    <a href="">
                        <div className="flex items-center justify-center xl:gap-5 gap-3 px-6 xl:py-5 bg-white/10 border border-white/20 rounded-xl lg:py-4 py-3.5 hover:bg-primary">
